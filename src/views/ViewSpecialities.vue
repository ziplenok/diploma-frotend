        ...
        >
          <table class="table is-hoverable">
            ...
            <tbody>
              ...
              <tr
                v-for="(program, i) in state.paginatedData"
                :key="state.paginatedData[i]"
                class="has-text-white"
                v-if="!eduProgramStore.loading"
              >
                <td class="shrink">{{ program.code }}</td>
                <td>{{ program.name }}</td>
                <td class="shrink">
                  {{ program.educationalGroupDto.prof1 }} -
                  {{ program.educationalGroupDto.prof2 }}
                </td>
              </tr>
            </tbody>
          </table>
...
onBeforeMount(() => {
  eduProgramStore.fetchAllEduPrograms();
});
...
const state = reactive({
  programsList: computed(() => {
    let filterList = eduProgramStore.list.allPrograms;

    if (selectedCombination.value !== "") {
      filterList = eduProgramStore.list.programsByCombination;
